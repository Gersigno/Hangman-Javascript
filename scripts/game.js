...
let wordToGuess; //This variable is, as you guessed it, the word that our player will need to guess..

let tries = 0; //This variable is used to check how many time our player picked a wrong letter
let difficulty = 1; //NEEDED to be between 1 and 3

let revealedLetters = new Array(); //an empty array to fill with guessed letters
...
function BuildDefaultGuess() {
	...
	const wordLetters = wordToGuess.toString().split("");

	//To avoid duplicate characters, We create a second array with filtered letters.
	let containedLetters = wordLetters.filter(
 ...
 */
function UpdateUI() {
	const element_wordToGuess = document.getElementById("WordToGuess")
...
function isLetterInWord(letter) {
	const currentGuess = document.getElementById("WordToGuess").innerHTML;
